...
def build_manifest():
    ...
    for pdf_path in glob.glob(os.path.join(PDF_DIR, "*.pdf")):
        ...
        toc_hits = []
        for p in range(min(15, len(doc))):  # Increased from 7 to 15 pages
        ...
        # 2) Only scan for specific section headers, not every mention
        print(f"  Scanning for specific section headers...")
        seen = set()
        for s in sections:
            seen.add((s["label"].lower(), s["start"]))
        
        # Only look for specific section patterns, not every mention
        section_patterns = [
            r"Evaluation Criteria:\s*([^\.]+)",
            r"Theme:\s*([^\.]+)",
            r"Sub[- ]Theme:\s*([^\.]+)",
            ...
            for pattern in section_patterns:
                for match in re.finditer(pattern, text, flags=re.I):
                    ...
                    if key in seen:
                        continue
                    seen.add(key)
                    sections.append({"label": label, "start": int(p + 1)})
